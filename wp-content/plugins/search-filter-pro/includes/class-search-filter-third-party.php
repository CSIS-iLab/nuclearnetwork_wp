...
class Search_Filter_Third_Party
{
				...
				foreach( $insert_data as $data_key => $data ) {
					...
					if( $attr_key !== false ) {
						...
						if(isset($product_attributes[$tax_name])) {
							...
							//}
							//actually don't only unset variation attributes, unset all attrubtes, as we'll assign them all to the vairiations
							//themselves
						}
	...
	public function sf_woo_post_cache_add_variation_product_data( $post ) {
		...
		//we first need to delete all variation data, otherwise we're duplicating!! - maybe hook into hte start of hte save function
