...
if ( $error_flag == 0 ) {
	...
	if ( $error_flag == 0 ) {
		...
		if ( $post_id ) {
			...
			// JS - Programmatically create user with Guest Authors Plugin.
			if ( function_exists( 'coauthors_posts_links' ) ) {
				$guest_author = array();
				$guest_author_extra = array();
			}
			...
			if ( isset($_POST[ 'form_custom_fields' ]) && !empty($_POST[ 'form_custom_fields' ]) ) {
				foreach ( $_POST[ 'form_custom_fields' ] as $key ) {
					...
					// JS - Programmatically add guest authors.
					if ( function_exists( 'coauthors_posts_links' ) && 'ap_author_bio' === $meta_key ) {
						$guest_author_extra['cap-description'] = $value;
					}
				}
			}
			...
			if ( function_exists( 'coauthors_posts_links' ) ) {
				...
				$author_id = $coauthors_plus->guest_authors->create( $guest_author );

				// Update Author Meta.
				foreach ( $guest_author_extra as $meta_key => $value ) {
					add_post_meta( $author_id, $meta_key, $value );
				}

				// Update Post Author.
				$coauthors = array( $guest_author['user_login'] );
